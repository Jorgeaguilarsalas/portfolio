...
const AUTOREPLY = {
  en: {
    ...
    body: [
      ...
      "— Case studies at jorgeag.com/work/",
      "— Operations in Motion (research publication) at jorgeag.com/motion/",
      "",
      "Best,",
    ],
  },
  de: {
    ...
    body: [
      ...
      "— Case Studies auf jorgeag.com/de/work/",
      "— Operations in Motion (Rechercheveröffentlichung) auf jorgeag.com/de/motion/",
      "",
      "Beste Grüße,",
    ],
  },
  es: {
    ...
    body: [
      ...
      "— Case studies en jorgeag.com/es/work/",
      "— Operations in Motion (publicación de investigación) en jorgeag.com/es/motion/",
      "",
      "Saludos,",
    ],
  },
...
const SIGNATURE = ["Jorge Aguilar", "Founder @ Salas UG", "Co-founder @ Nexio Fleet"];
...
async function sendViaResend(env, { to, subject, text, replyTo }) {
  const r = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from: `Jorge Aguilar <${env.CONTACT_FROM || FROM_DEFAULT}>`,
      to: [to],
      subject,
      text,
...
export async function handleContactSubmit(request, env) {
  ...
  if (env.TURNSTILE_SECRET_KEY) {
    ...
    if (!token || !(await checkTurnstile(env.TURNSTILE_SECRET_KEY, token, ip))) {
      return json({ ok: false, code: "captcha", message: "Spam check failed." }, 403);
    }
  }
  ...
  const notice = {
    ...
    subject: `[Contact form] ${data.inquiry_type || "general"} — ${data.name || "Anonymous"}`,
  ...
  if (data.email && hasResend) {
    try {
      ...
      await sendViaResend(env, { to: data.email, subject: ar.subject, text: ar.text });
    } catch (err) {
